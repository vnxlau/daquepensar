    ...
    return (
      <div className='row'>
        <div className='col-6'>
          ...
          <p><b>Morada:</b> {this.state.address.street}, <br /> {this.state.address.street2}, {this.state.address.postal_code} - {this.state.address.city}</p>
        </div>
        <div className='col'>
          <h3>Apoio</h3>
          <p><b>Plano:</b> { this.state.studentPlan.student_id } </p>
          <h4>Aula</h4>
          {this.state.classStudent.map((current_class, index) => {
            console.log(current_class);
            return (
            <p key={index} >
              <p><b>inicio:</b> {current_class.start_time} </p>
              <p><b>fim:</b> {current_class.end_time} </p>
            </p>
          );})}
        ...
        <div className='col'>
          <h3>Explicações</h3>
          {this.state.individualClass.map((indClass, index) => (
            <p key={index} >
              {indClass.teacher_id} - {indClass.date}<br />
              {indClass.start_time} - {indClass.end_time}
            </p>
          ))}
        </div>
      </div>
